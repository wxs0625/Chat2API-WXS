...
const providersAPI = {
  ...
  autoUpdateModels: (): Promise<{
    success: boolean
    results: { providerId: string; addedModels: string[] }[]
    error?: string
  }> =>
    ipcRenderer.invoke(IpcChannels.PROVIDERS_AUTO_UPDATE_MODELS),

  onModelsUpdated: (
    callback: (data: { providerId: string; addedModels: string[]; timestamp: number }) => void
  ): (() => void) => {
    const listener = (_: unknown, data: { providerId: string; addedModels: string[]; timestamp: number }) =>
      callback(data)
    ipcRenderer.on(IpcChannels.PROVIDERS_MODELS_UPDATED, listener)
    return () => {
      ipcRenderer.removeListener(IpcChannels.PROVIDERS_MODELS_UPDATED, listener)
    }
  },
}
...
const accountsAPI = {
  ...
  onStatusChanged: (callback: (account: Account) => void) => {
    const handler = (_event: Electron.IpcRendererEvent, account: Account) => callback(account)
    ipcRenderer.on(IpcChannels.ACCOUNTS_STATUS_CHANGED, handler)
    return () => ipcRenderer.removeListener(IpcChannels.ACCOUNTS_STATUS_CHANGED, handler)
  },
}
