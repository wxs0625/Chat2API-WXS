...
interface SettingsState {
  ...
  setAutoUpdateModels: (enabled: boolean) => void
  autoUpdateModelsIntervalHours: number
  setAutoUpdateModelsIntervalHours: (hours: number) => void
  maxConcurrentPerAccount: number
  setMaxConcurrentPerAccount: (max: number) => void
...
export const useSettingsStore = create<SettingsState>()(
  persist(
    (set, get) => ({
      ...
      setAutoUpdateModels: async (enabled) => {
        set({ autoUpdateModels: enabled })
        try {
          await window.electronAPI.config.update({ autoUpdateModels: enabled })
        } catch (error) {
          console.error('Failed to update autoUpdateModels:', error)
        }
      },
      autoUpdateModelsIntervalHours: 24,
      setAutoUpdateModelsIntervalHours: async (hours) => {
        set({ autoUpdateModelsIntervalHours: hours })
        try {
          await window.electronAPI.config.update({ autoUpdateModelsIntervalHours: hours })
        } catch (error) {
          console.error('Failed to update autoUpdateModelsIntervalHours:', error)
        }
      },
      maxConcurrentPerAccount: 1,
      setMaxConcurrentPerAccount: async (max) => {
        set({ maxConcurrentPerAccount: max })
        try {
          await window.electronAPI.config.update({ maxConcurrentPerAccount: max })
        } catch (error) {
          console.error('Failed to update maxConcurrentPerAccount:', error)
        }
      },
      minimizeToTray: true,
      ...
      fetchConfig: async () => {
        try {
          ...
          set({ 
            ...
            autoUpdateModels: config.autoUpdateModels ?? false,
            autoUpdateModelsIntervalHours: config.autoUpdateModelsIntervalHours ?? 24,
            maxConcurrentPerAccount: config.maxConcurrentPerAccount ?? 1,
