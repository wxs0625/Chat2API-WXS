...
export function ProxyConfigForm({ onConfigChange }: ProxyConfigFormProps) {
  ...
  const { proxyConfig, proxyStatus, setProxyConfig, saveAppConfig, startProxy, stopProxy, isLoading, error } = useProxyStore()
  ...
  const performSave = async (newPort: number, newHost: string) => {
    ...
    const success = await saveAppConfig({
      ...
      enableCors: newConfig.enableCors,
      corsOrigin: newConfig.corsOrigin,
    })
    ...
    if (success) {
      ...
      toast({
        ...
        description: error || t('proxy.configSaveFailed'),
        variant: 'destructive',
      })
    }
  }
