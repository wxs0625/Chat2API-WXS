...
function mapOAuthCredentials(providerId: string | undefined, credentials: Record<string, string>): Record<string, string> {
  ...
  if (oauthKey && credentials[oauthKey]) {
    ...
    if (fieldName) {
      ...
      // For Qwen AI, preserve cookies collected during in-app login
      if (providerId === 'qwen-ai') {
        const result: Record<string, string> = { [fieldName]: tokenValue }
        if (credentials['cookies']) {
          result['cookies'] = cookiesToString(credentials['cookies'])
          console.log('[mapOAuthCredentials] Preserved cookies for Qwen AI')
        }
        return result
      }
  ...
  // For Qwen AI, if token is already in the right field, still preserve cookies
  if (providerId === 'qwen-ai' && credentials['token']) {
    const result: Record<string, string> = { token: credentials['token'] }
    if (credentials['cookies']) {
      result['cookies'] = cookiesToString(credentials['cookies'])
      console.log('[mapOAuthCredentials] Preserved cookies for Qwen AI (no mapping needed)')
    }
    return result
  }
