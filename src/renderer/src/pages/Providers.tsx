...
export function Providers() {
  ...
  // Subscribe to account status changes pushed by the main process
  // (e.g. automatic re-check of abnormal accounts), so the UI updates live.
  useEffect(() => {
    if (!window.electronAPI?.accounts?.onStatusChanged) return

    const unsubscribe = window.electronAPI.accounts.onStatusChanged((account) => {
      const state = useProvidersStore.getState()
      const previous = state.getAccountById(account.id)

      state.updateAccount(account.id, {
        status: account.status,
        errorMessage: account.errorMessage,
        email: account.email,
      })

      // Refresh the "online" counter of the owning provider card.
      const updatedAccounts = useProvidersStore.getState().getAccountsByProvider(account.providerId)
      state.updateAccountCount(
        account.providerId,
        updatedAccounts.length,
        updatedAccounts.filter((a) => a.status === 'active').length
      )

      if (previous && previous.status !== account.status) {
        console.log(
          `[Providers] Account "${account.name}" status changed: ${previous.status} → ${account.status}`
        )
      }
    })
  ...
  // Subscribe to automatic model updates pushed by the main process
  // (auto model update scheduler / manual "Update Now"), refreshing the list.
  useEffect(() => {
    if (!window.electronAPI?.providers?.onModelsUpdated) return

    const unsubscribe = window.electronAPI.providers.onModelsUpdated(async (data) => {
      try {
        ...
        toast({
          ...
          description: `${provider?.name ?? data.providerId}: ${t(
            'settings.autoUpdateModelsAdded',
            { count: data.addedModels.length }
          )}`,
        })
      } catch (error) {
        console.error('[Providers] Failed to refresh providers after model update:', error)
      }
    })
