  ...
  async startInAppLogin(
    ...
    return new Promise((resolve) => {
      ...
      const tokenFoundHandler = async (event: { key: string; value: string; allCookies?: Record<string, string> }) => {
        ...
        // For Qwen AI, the token alone is insufficient — Alibaba Cloud risk control
        // requires browser cookies. Wait briefly after token capture so the cookie
        // source can collect allCookies before we validate and save credentials.
        if (providerType === 'qwen-ai') {
          if (!collectedTokens.token) {
            console.log('[OAuthManager] Waiting for Qwen AI token...')
            return
          }

          // If we already have cookies, validate immediately
          if (collectedTokens.cookies) {
            console.log('[OAuthManager] Qwen AI token and cookies collected, validating...')
            if (validationTimeout) {
              clearTimeout(validationTimeout)
            }
            validateAndComplete()
          ...
          // Wait briefly for cookies to be collected from the cookie token source
          if (validationTimeout) {
            clearTimeout(validationTimeout)
          }

          console.log('[OAuthManager] Qwen AI token found, waiting for cookies...')
          validationTimeout = setTimeout(() => {
            console.log('[OAuthManager] Proceeding with Qwen AI validation, has cookies:', !!collectedTokens.cookies)
            validateAndComplete()
          }, 3000)
          return
        }

        // For non-MiniMax/Mimo/Z.ai/Qwen AI providers, validate immediately when we have a token
