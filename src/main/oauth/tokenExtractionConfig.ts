...
export const TOKEN_EXTRACTION_CONFIGS: Record<ProviderType, TokenExtractionConfig> = {
  kimi: {
    ...
    // www.kimi.com redirects mainland China users only.
    loginUrl: 'https://www.kimi.ai',
    tokenSources: [
      {
        // New K3 site authenticates via the kimi-auth JWT cookie.
        type: 'cookie',
        key: 'kimi-auth',
      },
      {
        type: 'networkHeader',
        key: 'token',
        // Match both kimi.ai (K3) and legacy kimi.com API traffic.
        urlPattern: '*://*.kimi.*/*',
        extractPattern: '^Bearer\\s+(.+)$',
      },
    ],
    targetDomains: ['.kimi.ai', 'kimi.ai', '.kimi.com', 'kimi.com'],
    successUrlPatterns: [/kimi\.(ai|com)/i],
