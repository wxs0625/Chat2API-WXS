...
class StoreManager {
  ...
  private getEncryptionKey(): string | undefined {
    // IMPORTANT: This must NOT depend on safeStorage.isEncryptionAvailable().
    // electron-store (conf) performs pure AES-256-CBC encryption with this
    // fixed key and does not use the OS keyring. If key availability flipped
    // between runs (e.g. keyring/DBus transient state), the encrypted
    // data.json would be read as "unencrypted", fail JSON parsing, and
    // trigger the corruption-recovery path, silently wiping all accounts.
  ...
  updateAccount(id: string, updates: Partial<Account>): Account | null {
    ...
    console.log('[Store] Update account:', {
      id,
      updatesCredentials: Boolean(updates.credentials),
    })
    ...
    console.log('[Store] Account updated:', { id })
  ...
  /**
   * Merge Fetched Models into Provider Overrides (additive only)
   * Used by the automatic model update scheduler / manual sync.
   * Returns the names of newly added models.
   */
  mergeFetchedModels(
    providerId: string,
    ...
    const overrides = this.getUserModelOverrides()
    const providerOverrides = overrides[providerId]
    ...
    overrides[providerId] = result.overrides
  ...
  getEffectiveModels(providerId: string): EffectiveModel[] {
    ...
    const includedDisplayNames = new Set<string>()
    ...
    defaultModels.forEach(displayName => {
      if (!overrides.excludedModels.includes(displayName)) {
        ...
        includedDisplayNames.add(displayName)
      }
    })

    overrides.addedModels.forEach(customModel => {
      // Skip custom/auto-added models that already exist as default models
      // to avoid duplicates (e.g. after a manual model update).
      if (includedDisplayNames.has(customModel.displayName)) {
        return
      }
      effectiveModels.push({
