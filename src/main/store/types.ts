...
export interface AppConfig {
  ...
  proxyHost: string
  /** Whether CORS headers are enabled for proxy responses */
  enableCors: boolean
  /** Allowed CORS origin value */
  corsOrigin: string
  ...
  /** Whether provider model lists are refreshed automatically */
  autoUpdateModels: boolean
  /** Interval between automatic model refreshes, in hours */
  autoUpdateModelsIntervalHours: number
  /** Maximum simultaneous requests a single account may handle (default 1) */
  maxConcurrentPerAccount: number
  /** How long (ms) a request may wait for a free account slot before 429 */
  accountBusyTimeoutMs: number
}
...
/**
 * Merge freshly fetched models into the user model overrides, ADDITIVELY.
 *
 * Only models that are not already known to the provider (via built-in
 * `supportedModels`, previously added custom models, or user exclusions)
 * are appended to `addedModels`. Nothing is ever removed.
 *
 * @param overrides - Existing overrides for the provider (optional)
 * @param providerSupportedModels - Built-in supported model list of the provider
 * @param fetchedModels - Models fetched from the remote models API
 * @returns The merged overrides and the names of newly added models
 */
export function mergeFetchedModels(
  overrides: ProviderModelOverrides | undefined,
  providerSupportedModels: string[],
  fetchedModels: { displayName: string; actualModelId: string }[]
): { overrides: ProviderModelOverrides; addedModels: CustomModel[]; addedNames: string[] } {
  const existingDisplayNames = new Set(
    (overrides?.addedModels || []).map((model) => model.displayName)
  )
  const knownModels = new Set<string>([
    ...providerSupportedModels,
    ...(overrides?.excludedModels || []),
  ])

  const addedModels: CustomModel[] = []

  for (const fetched of fetchedModels) {
    const displayName = fetched.displayName?.trim()
    if (!displayName) continue
    if (existingDisplayNames.has(displayName)) continue
    if (knownModels.has(displayName)) continue

    addedModels.push({
      displayName,
      actualModelId: fetched.actualModelId?.trim() || displayName,
    })
  }

  return {
    overrides: {
      addedModels: [...(overrides?.addedModels || []), ...addedModels],
      excludedModels: [...(overrides?.excludedModels || [])],
    },
    addedModels,
    addedNames: addedModels.map((model) => model.displayName),
  }
}
...
export const DEFAULT_CONFIG: AppConfig = {
  ...
  autoUpdateModelsIntervalHours: 24,
  maxConcurrentPerAccount: 1,
  accountBusyTimeoutMs: 30000,
}
