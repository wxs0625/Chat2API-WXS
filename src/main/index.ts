...
// Workaround for black/blank window on Linux environments without proper GPU
// support (remote desktop / VNC / VMs): Chromium's GPU compositing can fail
// and render only the window background. Fall back to software rendering so
// the UI always paints. The UI is a lightweight React dashboard, so the
