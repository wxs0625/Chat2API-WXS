...
export class ProviderChecker {
  ...
  private static async checkKimiToken(token: string): Promise<TokenCheckResult> {
    try {
      ...
      const response = await axios.post(
        'https://www.kimi.ai/apiv2/kimi.gateway.order.v1.SubscriptionService/GetSubscription',
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  private static async checkQwenAiToken(token: string): Promise<TokenCheckResult> {
    try {
      ...
      // Upstream returns HTTP 200 even for expired sessions; the actual state
      // is in the response body: { success: false, data: { code: 'unauthorized', details: ... } }
      if (response.data?.success === false) {
        const details = response.data?.data?.details
        const code = response.data?.data?.code
        return { valid: false, error: details || `Validation failed: ${code || 'unauthorized'}` }
      }
