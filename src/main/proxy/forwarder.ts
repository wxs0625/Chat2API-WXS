...
/**
 * Detect authentication/session errors from upstream providers.
 * When such an error occurs, the account credential is invalid and
 * the account should be excluded from load balancing.
 */
function isAuthError(message: string): boolean {
  ...
  return (
    ...
    msg.includes('auth.token.invalid') ||
...
export class RequestForwarder {
  ...
  private async forwardDeepSeek(
    ...
    try {
      ...
      const latency = Date.now() - startTime
      return this.handleForwardError(account, provider, error, latency)
    }
  }
  ...
  private async forwardQwenAi(
    ...
    try {
      ...
      if (response.status >= 400) {
        ...
        // Exclude expired/invalid accounts immediately so they are not picked again
        if (response.status === 401 || response.status === 403) {
          AccountManager.updateStatus(account.id, 'error', errorMessage)
          console.log(`[Forwarder] Account ${account.id} (${provider.id}) marked as error due to HTTP ${response.status}`)
        }
      ...
      // Auth/session-expired errors: mark account invalid so load balancer
      // excludes it (the status scheduler re-checks it every 60s), and return
      // status 401 to avoid pointless retries with 5s delays.
      if (isAuthError(errorMessage)) {
        AccountManager.updateStatus(account.id, 'error', errorMessage)
        console.log(`[Forwarder] Account ${account.id} (${provider.id}) marked as error: ${errorMessage}`)
        return {
          success: false,
          status: 401,
          error: errorMessage,
  ...
  /**
   * Common error handling for provider forwards.
   * Auth/session-expired errors (or HTTP 401/403 from axios) mark the
   * account invalid so the load balancer skips it on the next request
   * (the status scheduler re-checks it automatically every 60s), and
   * return a <500 status to stop the retry loop immediately instead of
   * burning retries with 5s delays.
   */
  private handleForwardError(
    ...
    // Axios HTTP errors (e.g. "Request failed with status code 403")
    const axiosStatus = (error as AxiosError)?.response?.status

    if (isAuthError(errorMessage) || axiosStatus === 401 || axiosStatus === 403) {
      AccountManager.updateStatus(account.id, 'error', errorMessage)
      console.log(
        `[Forwarder] Account ${account.id} (${provider.id}) marked as error: ${errorMessage}`
      )
      return {
        success: false,
        status: axiosStatus && axiosStatus < 500 ? axiosStatus : 401,
        error: errorMessage,
        latency,
      }
    }
