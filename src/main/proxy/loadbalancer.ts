...
export class LoadBalancer {
  ...
  selectAccount(
    ...
    // Prefer accounts that still have free concurrency slots so bursts of
    // requests don't all pile onto the same busy account. If every account
    // is busy, fall back to the full candidate list; the per-account
    // concurrency guard will queue the request instead.
    const maxConcurrent = Math.max(1, storeManager.getConfig().maxConcurrentPerAccount ?? 1)
    const freeCandidates = candidates.filter(
      c => accountConcurrency.getInFlightCount(c.account.id) < maxConcurrent
    )
    if (freeCandidates.length > 0) {
      return this.pickFromCandidates(freeCandidates, strategy, preferredAccountId)
    }

    console.log(`[LoadBalancer] All ${candidates.length} candidate accounts busy; request will queue`)
    return this.pickFromCandidates(candidates, strategy, preferredAccountId)
  }

  /**
   * Pick a single account from candidates using strategy and preference.
   */
