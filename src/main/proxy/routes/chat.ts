...
router.post('/completions', async (ctx: Context) => {
  ...
  // Per-account concurrency guard: upstream providers typically serialize
  // requests per account — concurrent bursts onto the same account stall
  // every request. Acquire a slot (queuing briefly if needed); it is
  // released when the request finishes, the stream ends, or the client
  // disconnects.
  ...
  ctx.res.once('close', onQueueClose)

  const releaseSlot = await accountConcurrency.acquire(
    account.id,
    config.maxConcurrentPerAccount ?? 1,
    config.accountBusyTimeoutMs ?? 30000
  )
  ctx.res.removeListener('close', onQueueClose)

  if (!releaseSlot || clientDisconnected) {
    if (releaseSlot) {
      releaseSlot()
    } else {
      ctx.status = 429
  ...
  try {
    ...
    if (request.stream === true && result.stream) {
      // Release the concurrency slot when the stream finishes or the client
      // goes away early (releaseSlot is idempotent).
      result.stream.once('end', releaseSlot)
      result.stream.once('error', releaseSlot)
      ctx.res.once('close', () => {
        if (!ctx.res.writableEnded) {
      ...
      // Non-streaming responses are already fully received from upstream
      releaseSlot()
    }
  } catch (error) {
    releaseSlot()
