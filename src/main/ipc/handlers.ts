...
import { startAccountStatusScheduler } from '../store/accountStatusScheduler'
import {
  startModelUpdateScheduler,
  applyModelUpdateConfig,
  triggerModelUpdate,
} from '../store/modelUpdateScheduler'
...
export async function registerIpcHandlers(mainWindow: BrowserWindow | null): Promise<void> {
  ...
  // Auto re-check accounts whose status is abnormal (every 60s until active)
  startAccountStatusScheduler()

  // Auto-refresh provider model lists (if enabled in settings)
  startModelUpdateScheduler()
  ...
  ipcMain.handle(IpcChannels.CONFIG_UPDATE, async (_, updates: Partial<AppConfig>) => {
    ...
    // Apply auto model update schedule changes immediately
    if (
      updates.autoUpdateModels !== undefined ||
  ...
  ipcMain.handle(IpcChannels.PROVIDERS_SYNC_MODELS, async (_, providerId: string): Promise<{
    ...
    try {
      ...
      // Additively merge freshly fetched models into the user model overrides
      // so they persist across restarts and existing models are never removed.
      const fetchedModels = result.supportedModels.map((name) => ({
        displayName: name,
        actualModelId: result.modelMappings[name] || name,
      }))
      storeManager.mergeFetchedModels(providerId, fetchedModels)
  ...
  ipcMain.handle(IpcChannels.PROVIDERS_AUTO_UPDATE_MODELS, async (): Promise<{
    success: boolean
    results: { providerId: string; addedModels: string[] }[]
    error?: string
  }> => {
    try {
      const results = await triggerModelUpdate()
  ...
  ipcMain.handle(IpcChannels.PROVIDERS_UPDATE_MODELS, async (_, providerId: string): Promise<{
    ...
    try {
      ...
      // Persist new models additively into user overrides FIRST, because the
      // merge skips models already present in the provider's supportedModels.
      // Overrides survive app restarts (initializeDefaultProviders resets
      // supportedModels from the builtin config).
      const fetchedModels = supportedModels.map((name) => ({
        displayName: name,
        actualModelId: modelMappings[name] || name,
      }))
      storeManager.mergeFetchedModels(providerId, fetchedModels)

      // Also update the in-memory/persisted provider for immediate UI display.
      ProviderManager.update(providerId, {
        supportedModels,
        modelMappings,
      })
