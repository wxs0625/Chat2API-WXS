...
test('DeepSeek feature aliases are no longer seeded as global model mappings', () => {
  assert.deepEqual(Object.keys(DEFAULT_DEEPSEEK_MODEL_MAPPINGS), [])
  assert.equal(DEFAULT_DEEPSEEK_MODEL_MAPPINGS['deepseek-v4-flash-think'], undefined)
  assert.equal(DEFAULT_DEEPSEEK_MODEL_MAPPINGS['deepseek-v4-flash-search'], undefined)
  assert.equal(DEFAULT_DEEPSEEK_MODEL_MAPPINGS['deepseek-v4-flash-think-search'], undefined)
  assert.equal(DEFAULT_DEEPSEEK_MODEL_MAPPINGS['deepseek-v4-pro-think'], undefined)
  assert.equal(DEFAULT_DEEPSEEK_MODEL_MAPPINGS['deepseek-v4-pro-search'], undefined)
  assert.equal(DEFAULT_DEEPSEEK_MODEL_MAPPINGS['deepseek-v4-pro-think-search'], undefined)
...
test('custom model mappings are preserved without built-in defaults', () => {
...
test('DeepSeek default model mapping seeding preserves editable replacement semantics', () => {
  ...
  first['deepseek-v4-flash-search'] = {
    requestModel: 'deepseek-v4-flash-search',
    actualModel: 'mutated',
    preferredProviderId: 'custom',
  }
  assert.equal(createDefaultModelMappings()['deepseek-v4-flash-search'], undefined)
...
test('Kimi K3 and K2.6 model mappings reach the web chat request payload', () => {
  assert.deepEqual(kimiConfig.supportedModels, ['Kimi-K3', 'Kimi-K2.6'])
  assert.equal(kimiConfig.modelMappings?.['Kimi-K3'], 'kimi-k3')
  assert.equal(kimiConfig.modelMappings?.['Kimi-K2.6'], 'kimi-k2.6')
  ...
  assert.equal(k3Payload.scenario, 'SCENARIO_K3')
  assert.equal(k3Payload.message.scenario, 'SCENARIO_K3')
  assert.deepEqual(k3Payload.tools, [])
  assert.equal(k3Payload.options.thinking, false)
  assert.equal(k3Payload.options.reasoning_effort, undefined)

  // Explicit reasoning effort maps to Kimi proto enum names and implies thinking
  const effortPayload = createKimiChatPayload({
    model: 'kimi-k2.6',
    content: 'hello',
    enableWebSearch: false,
    enableThinking: false,
  ...
  assert.equal(effortPayload.options.thinking, true)
  assert.equal(effortPayload.options.reasoning_effort, 'HIGH')
...
test('domestic Qwen models match the web chat model ids captured from HAR', () => {
  ...
  assert.match(qwenAdapterSource, /'Qwen3\.8-Max': 'Qwen3\.8-Max'/)
...
test('Qwen AI defaults include the latest flagship plus preserved web models', () => {
  const expectedModels = [
    'Qwen3.8-Max',
    'Qwen3.7-Plus',
    'Qwen3.7-Max',
  ...
  assert.match(qwenAiAdapterSource, /'qwen3\.8-max':\s*'qwen3\.8-max'/)
  ...
  assert.doesNotMatch(qwenAiAdapterSource, /'qwen3\.6-35b':/)
  assert.doesNotMatch(qwenAiAdapterSource, /'qwen3\.6-27b':/)
  assert.doesNotMatch(qwenAiAdapterSource, /'qwen3-coder':/)
...
test('Z.ai default models match the latest chat.z.ai HAR model ids', () => {
  ...
  assert.match(zaiAdapterSource, /'GLM-5V-Turbo': 'GLM-5v-Turbo'/)
  assert.match(zaiAdapterSource, /const X_FE_VERSION = 'prod-fe-1\.1\.92'/)
